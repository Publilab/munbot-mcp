...
spec = importlib.util.spec_from_file_location(
    "mcp_core.orchestrator", os.path.join("mcp-core", "orchestrator.py")
)
...
def test_doc_query_calls_service(monkeypatch):
    ...
    assert (
        captured["params"]["pregunta"] == "cual es el mail de la licencia de conducir"
    )
...
def test_categoria_propagates_collection(monkeypatch):
    ...
    orchestrator.handle_document_query("sid", "pregunta", {}, [], "tramite")

    assert captured["params"]["categoria"] == "tramite"
    assert captured["collection"] == "tram_coll"


def test_categoria_ignores_unrecognized(monkeypatch):
    os.environ["RAG_COLLECTION_FAQ"] = "faq_coll"
    os.environ["RAG_COLLECTION_TRAMITES"] = "tram_coll"
    os.environ["RAG_COLLECTION_NORMATIVA"] = "norm_coll"
    ...
    assert "categoria" not in captured["params"]
    assert captured["collection"] == "norm_coll"
...
def test_full_info_summary(monkeypatch):
    ...
    def fake_oficinas(doc_id):
        return {
            "oficinas": [{"direccion": "Av 1", "horario": "8-12", "correo": "a@b.cl"}]
        }
    ...
    resp = orchestrator.orchestrate(
        "Quiero toda la información de la Licencia de Conducir"
    )
    assert "información específica" in resp["respuesta"].lower()
    assert "licencia de conducir" in resp["respuesta"].lower()
