...
def orchestrate(user_input: str, extra_context: Optional[Dict[str, Any]] = None, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    SALUDOS = [
        "hola", "buenos días", "buenas tardes", "buenas noches", "saludos", "hey", "holi", "hello", "hi"
    ]
    DESPEDIDAS = [
    ...
    PREGUNTAS_PERSONALES = [
        "quién eres", "quien eres", "eres un robot", "qué eres", "que eres", "cuántos años tienes", "cuantos años tienes", "eres humano", "eres una persona"
    ]
    texto = user_input.strip().lower()
    if texto in SALUDOS:
        return {"respuesta": "¡Hola! ¿En qué puedo ayudarte hoy?", "session_id": session_id or str(uuid.uuid4())}
    ...
    if not validate_against_schema(params, schema):
        return {"error": "Faltan parámetros obligatorios para esta acción", "session_id": session_id}
    ...
    delete_session(session_id)
    # Ajuste para asegurar que 'respuesta' siempre sea string plano
    if isinstance(response, dict):
        if "respuesta" in response:
            ...
            return {"respuesta": str(response).strip(), "session_id": session_id}
